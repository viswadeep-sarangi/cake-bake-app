...
from cake_bake_app.db import (
    add_cake_preference_db,
    add_employee_db,
    all_cake_preferences_db,
    all_cake_responsibilities_db,
    get_all_employees_db,
...
async def add_employee(request_body: EmployeesModel, session=Depends(get_session)):
    ...
    return {
        "message": f"{request_body.name} added to database   {employee_orm.as_dict()}"
    }
...
async def submit_preferences(
    request_body: CakePreferencesModel, session=Depends(get_session)
):
    """Submit cake preferences"""
    cake_pref_orm = CakePreferences(**dict(request_body))
    print(request_body)
    print(cake_pref_orm)
    cake_pref_orm = add_cake_preference_db(
        cake_preference=cake_pref_orm, session=session
    )
    return {
        "message": f"Cake preferences for {request_body.name} added to database     {cake_pref_orm.as_dict()}"
    }


@router.post("/employee_preferences", summary="Get cake preference for an employee")
async def get_employee_cake_preference(
    request_body: EmployeeRequestModel, session=Depends(get_session)
):
    return get_cake_preference_db(request_body.name, session)


@router.get("/preferences", summary="Get all cake preferences")
async def get_all_cake_preferences(
    session=Depends(get_session),
) -> List[CakePreferences]:
...
@router.post(
    "/employee_baking_responsibillity", summary="Get cake baking responbilities"
)
